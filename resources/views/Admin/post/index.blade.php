                                ...
                                <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" aria-describedby="example1_info">
                                                ...
                                                <tbody>
                                                    @foreach($ptd as $key=>$data)
                                                    <tr class="odd">
                                                        <td class="dtr-control sorting_1" tabindex="0">{{++$key}}</td>
                                                        <td>{{$data->category->Category_name??'none'}}</td>
                                                        <td>{{$data->subcategory->subcategory_slug??'none'}}</td>
                                                        <td>{{$data->users->name ??'none'}}</td>
                                                        <td>{{$data->title ??'none'}}</td>
                                                        <td>{{$data->post_date ??'none'}}</td>


                                                        <td class="d-flex gap-5">
                                                            <a href="{{route('post.edits',$data->id)}}" class="btn btn-primary">Edit</a>
                                                            <a href="javascript:void(0);" data-href="{{route('post.delete',$data->id??'none')}}', ['id' => $data->id]) }}" class="btn btn-danger delete">Delete</a>
                                                        </td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12 col-md-5">
                                            <div class="dataTables_info" id="example1_info" role="status" aria-live="polite">Showing 2 to 10 of 57 entries</div>
                                        </div>
